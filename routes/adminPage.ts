...
//拎 users
adminPageRoutes.get("/user/admin", async (req, res) => {
    try {


        const admin_result = await client.query(/*sql*/'select * from users order by username ;');
...
//改 user
adminPageRoutes.put("/user/update", async (req, res) => {
    try {
