...
import { client, io, newForm } from "../createEvents";
...
messageRoutes.post("/messages/create", (req: Request, res: Response) => {
    try {
        newForm.parse(req, async (err, fields, files) => {
            ...
            const messages_comment_result = await client.query(/*sql*/`INSERT INTO messages (heading,comment) VALUES ($1,$2) RETURNING id`, [messageHeading, messageContent]);
            ...
            if (Array.isArray(files.images_upload)) {
                for (let i = 0; i < files.images_upload.length; i++) {
                    ...
                    messages_images_result = await client.query(/*sql*/`INSERT INTO message_images (filename,message_id) VALUES ($1,$2) RETURNING id`, [imageName, messagesId]);
                }
...
messageRoutes.get('/messages/get', async (req, res) => {
    try {
        ...
        const messages_comment_result = await client.query(/*sql*/`with favorite_count as ( select messages.id, count('messages.id') as favorite_count from messages left join user_favorite_messages on messages.id = user_favorite_messages.message_id group by messages.id ) 
...
messageRoutes.get('/message_images/get', async (req, res) => {
    try {
        const messages_images_result = await client.query(/*sql*/'SELECT * from message_images');
...
messageRoutes.get('/message_like/get', async (req, res) => {
    try {
        ...
        const messages_like_result = await client.query(/*sql*/`SELECT count(*) from user_favorite_messages WHERE message_id = 56`);
...
messageRoutes.delete("/message/update", async (req, res) => {
    try {
        ...
        await client.query('delete from user_favorite_messages where message_id = $1', [Number(index)])
        await client.query('delete from message_images where message_id = $1', [Number(index)])
...
messageRoutes.post("/message/like", async (req, res) => {
    try {
        ...
        let checkLikeResult = await client.query(/*sql*/`SELECT * FROM user_favorite_messages WHERE message_id=($1)`, [Number(index)]);
        ...
        if (checkLikeResult.rowCount > 0) {
            await client.query(/*sql*/`delete from user_favorite_messages where message_id = $1`, [Number(index)]);
        } else {
            await client.query(/*sql*/`INSERT INTO user_favorite_messages (message_id) VALUES ($1) RETURNING id`, [Number(index)]);
